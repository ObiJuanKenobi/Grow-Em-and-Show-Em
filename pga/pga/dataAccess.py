...
class DataAccess:
    ...
    def addQuizAttempt(self, coursename, username, passed):
        ...
        data = self._cursor.execute("SELECT Attempts FROM Quizzes WHERE Course_Name = %s AND Username = %s", (coursename, username))
        #If data is empty, this condition evaluates to True
        if not data:
            self._cursor.execute("INSERT into Quizzes (Course_Name, Username, Attempts, Passed) values (%s, %s, %s, %s)", (coursename, username, 1, passed))
        else:
            self._cursor.execute("UPDATE Quizzes SET Attempts = Attempts + 1, Passed = %s WHERE Course_Name = %s AND Username = %s", (passed, coursename, username))
